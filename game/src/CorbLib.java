...
public class CorbLib {
  ...
  public static void drawBox(Graphics g, int x, int y, int w, int h, Color c){
    ...
    if(g==null) return;
    // handle null color
    Color usingColor= (c!=null)? c : CorbLib.getColorByType('\0');
  ...
  public static void drawBoxRect(Graphics g, Rectangle drawSpace, Color c){
    CorbLib.drawBox(g, drawSpace.x,drawSpace.y,drawSpace.width,drawSpace.height, c);
  }
  ...
  public static void drawDefaultRectangle(Graphics g, Rectangle drawSpace){
    CorbLib.drawBox(g, drawSpace.x,drawSpace.y,drawSpace.width,drawSpace.height, null);
  }
  ...
  public static void drawPixels( Graphics g, char[][] pixelTypeArr, Rectangle drawSpace ){
    ...
    if(pixelTypeArr.length==0 || pixelTypeArr[0].length==0) {
      ...
      CorbLib.drawDefaultRectangle(g,drawSpace);
      return;
    }
    ...
    for( int x = 0; x < cols; x++ ){
      for( int y = 0; y < rows; y++ ){
        ...
        drawBox(
          ...
          CorbLib.getColorByType(pixelTypeArr[x][y]) // get color
          );
      }
    }
  }
  ...
  public static char[][] stringToPixelTypeArray(String shapeIn){
    ...
    // get the size
    int size = CorbLib.getSizeByStringLength( shapeIn.length() );
  ...
  public static int getSizeByStringLength( int shapeStringLengthIn ){
    ...
    System.err.println("somehow didnt find a correct size bracket for CorbLib.getSizeByStringLength(int), V E R Y  S P O O K Y");
    return 1;
  }
